...
def get_ydl_opts(extra=None):
    """Base yt-dlp options with cookie support and anti-bot headers."""
    ...
    # Only pass cookiefile if it actually exists and has content
    if cookie_path and os.path.isfile(cookie_path) and os.path.getsize(cookie_path) > 10:
...
class AudioController(object):
    ...
    async def process_song(self, track):
        ...
        try:
            with yt_dlp.YoutubeDL(get_ydl_opts()) as ydl:
                try:
                    r = ydl.extract_info(track, download=False)
    ...
    def search_youtube(self, title):
        ...
        return "https://www.youtube.com/watch?v={}".format(entries[0]['id'])
